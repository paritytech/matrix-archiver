...
Suited for CI / GitHub‑Actions.  LISTEN_MODE defaults to “all”.
"""
...
# ────────────── credentials (mc_creds.json + ./store/) ─────────────────
...
def run_stream(cmd):
    ...
    for _ in _stream(p.stderr): pass
    out = "".join(_stream(p.stdout))
    if p.wait():
...
# ─────────────── human‑friendly room title  (fixed) ────────────────────
room_title = ROOM
try:
    ...
    if meta:
        for key in ("display_name", "name",        # ← NEW
